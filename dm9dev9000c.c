...
static const struct net_device_ops dm9k_netdev_ops = {
	...
	//.ndo_set_features	= dm9000_set_features,      //DM9KS_RCSR已设置过了,不需要
	.ndo_validate_addr	= eth_validate_addr,
	.ndo_set_mac_address	= eth_mac_addr,
        .ndo_get_stats 	 = dmfe_get_stats,         //添加该句,获取统计信息
...
int __init dmfe_probe1(struct net_device *dev)
{
	...
	do {
		outb(DM9KS_VID_L, iobase);      //CMD引脚置0,写入要读的地址值
...
int __init dm9000c_init(void)
{
       ...
       iobase       =(int)ioremap(0x20000000, 4096);    //获取DM9000C 基地址
       ...
       /*  ST4 [19]        :    0  =未使用 UB/LB 
         ...
         */
        *BWSCON &=~(0xF<<16);
        *BWSCON |= (0x01<<16);         //关闭UB/LB,禁止WAIT信号,设置16位宽数据
